...
def video_play():
    ...
    if ret:
        ...
        # 적색만 인식하여 화면에 출력하는 영상 (video_red)
        # 녹색만 인식하여 화면에 출력하는 영상 (vidoe_green)
        video_red = cv2.bitwise_and(frame_r, video_red2)
        ...
        if np.isnan(px_b) == False and np.isnan(py_b) == False:
            # 공격팀 승리 조건
            if int(px_b) >= int(lz_x_r) - 5 and int(px_b) <= int(lz_x_r) + 5 and int(py_b) >= int(lz_y_r) - 5 and int(py_b) <= int(lz_y_r) + 5:
                win.set("공격팀 승리!")
                judge = True
        if (int(lz_x_r) != 0 and int(lz_y_r) != 0) and (int(lz_x_g) != 0 and int(lz_y_g) != 0):
            if int(lz_x_g) >= int(lz_x_r) - 5 and int(lz_x_g) <= int(lz_x_r) + 5 and int(lz_y_g) >= int(lz_y_r) - 15 and int(lz_y_g) <= int(lz_y_r) + 15:
                win.set("수비팀 승리!")
                judge = True
